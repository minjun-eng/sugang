...
function setup() {
  ...
  // 입력 이벤트 디바운싱(Debounce) 적용으로 성능 최화
...
async function requestDurableStorage() {
  try {
    ...
    // 저장소 지속성은 브라우저 힌트이므로 실패해도 정상 동작
  }
}
...
function saveSavedSchedules() {
  try {
    ...
    showToast("브라우저 저장 공간이 가득 찼거나 사용할 수 없습니다.");
  }
}
...
function renderResults() {
  ...
  if (els.visibleCount) els.visibleCount.textContent = visibleRows.length.toLocaleString("ko-KR");
  if (els.resultNote) els.resultNote.textContent = `${Math.min(renderLimit, visibleRows.length).toLocaleString("ko-KR")} / ${visibleRows.length.toLocaleString("ko-KR")}`;
  if (els.more) els.more.hidden = shown.length >= visibleRows.length;
}
...
function openSaveNameModal(mode) {
  ...
  if (els.saveNameTitle) els.saveNameTitle.textContent = isFirstSave ? "시간표 저장" : "다른 이름으로 저장";
  if (els.confirmSaveName) els.confirmSaveName.textContent = "저장";
  if (els.scheduleNameInput) els.scheduleNameInput.value = isFirstSave ? nextScheduleName() : defaultSaveName();
  if (els.saveNameModal) els.saveNameModal.hidden = false;
  window.requestAnimationFrame(() => {
...
function closeSaveNameModal() {
  if (els.saveNameModal) els.saveNameModal.hidden = true;
}
...
function openLoadScheduleModal() {
  ...
  if (els.loadModal) els.loadModal.hidden = false;
}

function closeLoadScheduleModal() {
  if (els.loadModal) els.loadModal.hidden = true;
}

function renderSavedScheduleList() {
  if (!els.savedScheduleList) return;
...
function openRenameScheduleModal(id) {
  ...
  if (els.saveNameTitle) els.saveNameTitle.textContent = "시간표 이름 변경";
  if (els.confirmSaveName) els.confirmSaveName.textContent = "변경";
  if (els.scheduleNameInput) els.scheduleNameInput.value = schedule.name;
  if (els.saveNameModal) els.saveNameModal.hidden = false;
  window.requestAnimationFrame(() => {
    els.scheduleNameInput?.focus();
    els.scheduleNameInput?.select();
...
function renderSelectedList(items, conflicts) {
  ...
  if (els.selectedCount) els.selectedCount.textContent = items.length.toLocaleString("ko-KR");
  if (els.creditCount) els.creditCount.textContent = credits.toLocaleString("ko-KR", { maximumFractionDigits: 1 });
  if (els.conflictCount) els.conflictCount.textContent = conflicts.size.toLocaleString("ko-KR");
  if (els.selectedNote) els.selectedNote.textContent = items.length ? `${items.length}개 선택` : "비어 있음";
  if (els.conflictBanner) els.conflictBanner.hidden = conflicts.size === 0;
  updateDocumentState();

  if (!els.selectedCourses) return;
...
function renderDetail(row) {
  if (!els.detailTitle || !els.detailList) return;
  if (!row) {
  ...
  els.detailList.replaceChildren(...detailRows);
}
...
function downloadTimetableImage() {
  ...
  ctx.fillText(`과목 ${items.length}개 · 학점 ${els.creditCount?.textContent || 0} · 마지막 업데이트 ${formatUpdatedAt(rawData.meta?.fetchedAt || "")}`, 72, 128);
